...
class RegressionReport:
    ...
    def compare(self, baseline: dict) -> dict:
        cur = self.summary()
        return {f"{k}_delta": cur[k] - baseline[k] for k in baseline if k in cur}


class RegressionRunner:
    def __init__(self, set_path: Path, query_fn: Callable[[str], dict]) -> None:
        self.items = [json.loads(line) for line in Path(set_path).read_text().splitlines() if line]
        self.query_fn = query_fn

    @classmethod
    def from_items(cls, items: list[dict], query_fn: Callable[[str], dict]) -> "RegressionRunner":
